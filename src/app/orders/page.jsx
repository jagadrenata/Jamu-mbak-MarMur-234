 ...
 async function handleSearch(e) {
  ...
  try {
   const res = await fetch(
    `/api/orders?id=${encodeURIComponent(identifier.trim())}`
   );
 ...
 return (
  <div className='py-16 px-4 text-center'>
   ...
   <p className='text-sm mb-6 opacity-70' style={{ color: C.text }}>
    Masukkan ID pesanan kamu untuk melihat statusnya, atau{" "}
    <Link
     href='/login'
    ...
    untuk melihat semua pesanan sekaligus.
   </p>
   <form onSubmit={handleSearch} className='flex gap-2 max-w-md mx-auto'>
    <input
     value={identifier}
     onChange={e => setIdentifier(e.target.value)}
     placeholder='Contoh: ABC-202403...'
    ...
    <button
     type='submit'
     disabled={loading}
     className='px-4 py-2.5 text-sm font-medium flex items-center gap-2 transition-opacity disabled:opacity-60'
     ...
     <div className='text-center px-4'>
      ...
      >
       {user
        ...
        : "Pesanan tetap aman tanpa login. Lacak menggunakan ID pesanan, atau login untuk melihat semua pesanan."}
      </p>
     </div>
    </div>
