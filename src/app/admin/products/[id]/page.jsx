...
export default function ProductDetailPage() {
  ...
  const loadProduct = useCallback(async () => {
    ...
    try {
      ...
      const detailRes = await fetch(
        `/api/data/products?id=${id}&detail=1`
      ).then(r => r.json());
  ...
  function setVField(f, v) {
    setVariantForm(prev => ({ ...prev, [f]: v }));
  }
  function setIField(f, v) {
    setInfoForm(prev => ({ ...prev, [f]: v }));
  }
    ...
    return (
      <div className='text-center py-20 text-gray-400'>
        ...
        <button
          ...
          className='mt-4 text-sm text-cream-600 hover:underline'
        >
          ← Kembali ke daftar
        </button>
      </div>
  ...
  return (
    <div>
      ...
      <div className='flex items-start gap-4 mb-6'>
        <button
          onClick={() => router.push("/admin/products")}
          className='mt-1 p-2 text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors'
        >
          ...
          <div className='flex items-center gap-3 flex-wrap'>
            ...
            <div>
              ...
              <div className='flex items-center gap-2 mt-0.5'>
                ...
                {product.sku && (
                  <span className='font-mono text-xs text-gray-400 bg-gray-100 px-1.5 py-0.5 '>
                    {product.sku}
                  </span>
                )}
                {product.category && (
        ...
        {TABS.map(t => {
          ...
          return (
            ...
            >
              ...
              {t.key === "variants" && (
                <span
                  className={`text-xs px-1.5 py-0.5 rounded-full ${isActive ? "bg-cream-100 text-cream-700" : "bg-gray-100 text-gray-500"}`}
                >
      ...
      {tab === "info" && (
        <Card>
          <div className='flex items-center justify-between mb-5'>
            ...
            {!infoEditing ? (
              <Button onClick={() => setInfoEditing(true)} variant='secondary'>
          ...
          {infoEditing ? (
            <div className='space-y-4 max-w-2xl'>
              <Input
                label='Nama Produk *'
                value={infoForm.name}
                onChange={e => setIField("name", e.target.value)}
              />
              <div className='grid grid-cols-2 gap-4'>
                <Input
                  label='Slug'
                  value={infoForm.slug}
                  onChange={e => setIField("slug", e.target.value)}
                />
                <Input
                  label='SKU'
                  value={infoForm.sku}
                  onChange={e => setIField("sku", e.target.value)}
                />
              </div>
              <Select
                label='Kategori'
                value={infoForm.category_id}
                onChange={e => setIField("category_id", e.target.value)}
              >
              ...
              <Textarea
                ...
                onChange={e => setIField("description", e.target.value)}
                rows={4}
              />
              <Select
                label='Status'
                value={infoForm.status}
                onChange={e => setIField("status", e.target.value)}
              >
              ...
              {[
                ...
                <div key={row.label}>
                  ...
                  typeof row.value === "number" ? (
                    <div
                      className={`text-sm text-gray-800 ${row.mono ? "font-mono" : ""}`}
                    >
                      {row.value}
                    </div>
                  ) : (
        ...
        <Card>
          ...
          {(product.variants || []).length === 0 ? (
            <div className='text-center py-12 text-gray-400 border-2 border-dashed border-gray-200'>
              ...
              <p className='text-sm'>
                Belum ada variant. Tambahkan variant untuk mengatur harga dan
                stok.
              </p>
              <button
                onClick={openCreateVariant}
                className='mt-3 text-sm text-cream-600 hover:underline'
              >
            ...
            >
              {(product.variants || []).map(v => (
                <Tr key={v.id}>
                    ...
                    <div className='flex items-center gap-3'>
                      {v.images?.[0] ? (
                        <img
                          src={v.images[0].url}
                          alt={v.name}
                          className='w-9 h-9 object-cover  border border-gray-100'
                        />
                      ) : (
                        <div className='w-9 h-9 bg-gray-100 border border-gray-100 flex items-center justify-center'>
                  ...
                  <Td>
                    <span
                      className={`text-sm font-medium ${v.quantity === 0 ? "text-red-500" : v.quantity < 5 ? "text-cream-600" : "text-gray-800"}`}
                    >
                      {v.quantity}
                    </span>
                  </Td>
                  ...
                  <Td>
                    <div className='flex gap-1'>
                      <button
                        onClick={() =>
                          router.push(`/admin/products/${id}/variants/${v.id}`)
                        }
                        className='p-1.5 text-gray-400 hover:text-cream-600 hover:bg-cream-50  transition-colors'
                      ...
                      <button
                        ...
                        className='p-1.5 text-gray-400 hover:text-cream-700 hover:bg-cream-50  transition-colors'
                        title='Edit variant'
                      >
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant='secondary' onClick={() => setVariantModal(false)}>
              Batal
            </Button>
            <Button onClick={saveVariant} disabled={variantSaving}>
              {variantSaving ? "Menyimpan..." : "Simpan"}
            </Button>
          </>
        }
      >
        <div className='space-y-4'>
          <div className='grid grid-cols-2 gap-4'>
            ...
            <Input
              ...
              onChange={e => setVField("sku", e.target.value)}
            />
          </div>
          <div className='grid grid-cols-2 gap-4'>
            <Input
              ...
              onChange={e => setVField("price", e.target.value)}
            />
            <Input
              label='Harga Pokok (Rp)'
              type='number'
              value={variantForm.cost_price}
              onChange={e => setVField("cost_price", e.target.value)}
            />
          </div>
          <div className='grid grid-cols-2 gap-4'>
            <Input
              ...
              onChange={e => setVField("quantity", e.target.value)}
            />
            <Input
              label='Berat (gram)'
              type='number'
              value={variantForm.weight}
              onChange={e => setVField("weight", e.target.value)}
            />
          </div>
          <div>
            <label className='block text-sm font-medium text-gray-700 mb-1'>
              Atribut (JSON opsional)
            </label>
            <textarea
              className='w-full px-3 py-2 border border-gray-200  text-sm font-mono focus:outline-none focus:ring-2 focus:ring-cream-400'
              rows={3}
          ...
          <div className='flex items-center gap-3'>
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform -full bg-white shadow transition-transform ${variantForm.is_active ? "translate-x-6" : "translate-x-1"}`}
              />
            </button>
